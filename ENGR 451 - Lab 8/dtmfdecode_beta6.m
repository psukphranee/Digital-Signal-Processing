...
function output = dtmfdecode_beta6(x, fs)
    ...
    for i=0:slide_amount
        ...
        if inside_digit_flag==false
            ...
            if energy < 22 || (i==slide_amount)
                % if we are exiting a digit, push the data to the output stack
                % and clear the buffer
                inside_digit_flag = false;

                % need to figuree out digit. use findpeaks function.
                % findpeaks returns several local minima. so we subtract
                % by 0.5 and zero out negative values to prevent returning
                % noise peaks.
                temp = frame_fft_buffer - 0.5;
                temp(temp < 0) = 0;
                [peaks, peak_indices] = findpeaks(temp);
                % convert first peak to frequency
                row_freq = ((peak_indices(1)-1)/M)*fs;
                col_freq = ((peak_indices(2)-1)/M)*fs;
                digit_row_col = [row_freq, col_freq];
                
                [keypad_row, keypad_col] = freq_to_index(digit_row_col);
                digit = get_digit([keypad_row, keypad_col]);
