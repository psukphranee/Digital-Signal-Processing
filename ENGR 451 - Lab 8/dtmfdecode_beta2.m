...
function output = dtmfdecode_beta1(x, fs)
    ...
    for i=0:slide_amount
        ...
        % toggle entry or exit out of digit
        % if we are outside of a digit, then we will be in a digit if
        % energy is above the following
        if inside_digit_flag==false && energy > 45
            inside_digit_flag=true;
        elseif inside_digit_flag==true && energy < 20
            inside_digit_flag = false;
        end
        ...
        plot(frame);
        title(sprintf('Frame %d: Inside Digit = %d, Energy = %f', i, inside_digit_flag, energy));
        xlabel('Sample');
        ylabel('Amplitude');
        pause(1.5);
